...
import { apiGetCategories, Category } from '@/features/admin/services/category-service';
import { apiGetMenuItems, MenuItem } from '@/features/admin/services/menu-item-service';
import { apiGetTables, Table } from '@/features/admin/services/table-service';
import BillingView from '@/features/waiter/views/billing-view';
...
export default function WaiterDashboard() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            setCategories(extractList(cats?.data ?? cats));
            setMenuItems(extractList(items?.data ?? items));
            setTables(extractList(tbls?.data ?? tbls));
        } catch (err) {
    ...
    const fetchActiveOrder = async (table: Table) => {
        ...
        try {
            ...
            if (table?.status === 'OCCUPIED') {
                try {
                    const allOrdersRes = await api.get(`/orders?page=1&limit=50`);
    ...
    useEffect(() => {
        if (activeOrder?._id && activeOrder?.status !== 'COMPLETED') {
            const statusCheckInterval = setInterval(async () => {
                try {
                    const res = await api.get(`/orders/${activeOrder._id}`);
                    if (res.data.data) {
    ...
    const sendToKitchen = async () => {
        ...
        try {
            ...
            if (activeOrder) {
                await api.put(`/orders/${activeOrder._id}/append`, orderData);
            ...
            if (selectedTable) {
                ...
                const updatedTables = await apiGetTables();
                const updatedTablesList = extractList(updatedTables?.data ?? updatedTables);
    ...
    return (
        <div className="flex flex-col h-screen overflow-hidden bg-white font-dm-sans">
            <header className="h-16 px-4 md:px-6 lg:pl-6 lg:pr-[420px] flex items-center justify-between bg-white border-b border-zinc-100 z-20">
                <div className="flex items-center gap-4 md:gap-8 flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-lg md:text-xl font-bold text-zinc-900 tracking-tight">DineSmart</span>
                    </div>
                </div>
            ...
            <div className="flex-1 overflow-hidden relative lg:pr-[420px]">
                <div className={`h-full overflow-y-auto no-scrollbar pt-6 ${isBillSummaryOpen ? 'pb-[390px] md:pb-[430px] lg:pb-6' : 'pb-[110px] md:pb-[120px] lg:pb-6'} lg:border-r lg:border-zinc-200 space-y-8`}>
                                    ...
                                    {showTableFilterMenu && (
                                        <div className="absolute top-10 right-0 w-48 bg-white rounded-xl border border-zinc-200 shadow-xl z-50 overflow-hidden">
                                            <div className="p-2 border-b border-zinc-100">
                                                <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest px-2">Filter by Status</p>
                                            </div>
                                            <div className="p-1">
                    ...
                    <section className="px-6 space-y-6 animate-in slide-in-from-bottom-4 duration-400">
                            <div className="flex gap-4 overflow-x-auto no-scrollbar pt-2 pb-2">
                                <button
                                    ...
                                    className={`flex items-center gap-3 pl-3 pr-4 py-3 rounded-2xl transition-all min-w-[136px] border
                                ...
                                >
                                    ...
                                    <div className="text-left">
                                        ...
                                        <p className={`text-[10px] mt-1 font-semibold ${activeCategory === 'All Menu' ? 'text-white/80' : 'text-zinc-400'}`}>
                                ...
                                {categories.map(cat => (
                                    ...
                                    >
                                        ...
                                        <div className="text-left">
                                            ...
                                            <p className={`text-[10px] mt-1 font-semibold ${activeCategory === cat.name ? 'text-white/80' : 'text-zinc-400'}`}>
                                ...
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <div className="relative group flex-1 sm:w-64">
                                        ...
                                        <input
                                            ...
                                            className="h-10 w-full pl-10 pr-4 bg-white border border-zinc-300 rounded-lg text-sm font-semibold text-zinc-900 placeholder:text-zinc-400 hover:border-zinc-400 focus:border-[#FF5C00] transition-all outline-none"
                                        />
                                    </div>
                                        ...
                                        <button 
                                            ...
                                            className={`flex items-center gap-2 text-sm font-bold transition-colors px-3 py-2.5 rounded-lg border whitespace-nowrap
                                                ${menuFilter !== 'ALL' 
                                                    ...
                                                    : 'text-zinc-700 hover:text-zinc-900 border-zinc-300 hover:border-zinc-400 bg-white'}`}
                                        >
                                    ...
                                    {showMenuFilterMenu && (
                                        <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl border border-zinc-200 shadow-xl z-50 overflow-hidden">
                                            <div className="p-2 border-b border-zinc-100">
                                                <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest px-2">Filter Menu</p>
                                            </div>
                                            <div className="p-1">
                                                <button
                                        ...
                                        <>
                                            {paginatedItems.map(item => (
                                                <div
                                                    ...
                                                    className="group bg-white rounded-xl p-3.5 border border-zinc-100 hover:border-zinc-200 hover:bg-zinc-50 transition-all cursor-pointer relative overflow-hidden"
                                                >
                                            ...
                                            {/* Pagination Controls */}
                                            {totalPages > 1 && (
                                                <div className="col-span-full flex items-center justify-center gap-2 mt-6 pb-4">
                                                    <button
                ...
                <div className={`fixed inset-x-0 bottom-0 z-30 flex flex-col bg-white border-t border-zinc-200 shadow-[0_-8px_24px_rgba(0,0,0,0.08)] transition-all duration-300
                    ${isBillSummaryOpen ? 'h-[72vh] md:h-[68vh]' : 'h-[126px] md:h-[132px]'}
                    ...
                    <div className="h-16 px-4 md:px-5 lg:px-8 border-b border-zinc-100 flex items-center">
                        <div className="flex items-center justify-between w-full">
                            <h3 className="text-lg lg:text-xl font-bold text-zinc-900">Bill Summary</h3>
                            <div className="flex items-center gap-2">
                                <button
                    ...
                    {!isBillSummaryOpen && (
                        <div className="lg:hidden px-4 md:px-5 pb-3 space-y-1.5 border-b border-zinc-100">
                            <div className="flex items-center justify-between text-[11px] font-semibold text-zinc-500">
                                ...
                                <span>Subtotal: NRs. {subtotal.toLocaleString()}</span>
                            </div>
                    ...
                    <div className={`${isBillSummaryOpen ? 'flex' : 'hidden'} lg:flex flex-col min-h-0 flex-1 px-4 md:px-5 lg:px-8 pb-0 lg:pb-8`}>
                        <div className="mb-5 mt-3 space-y-2 border-b border-zinc-100 pb-5">
                            <div className="flex items-center gap-2">
                                <div className="w-4 flex justify-center"><Utensils size={14} className="text-[#FF5C00]" /></div>
                            ...
                            <div className="flex items-center gap-2">
                                <div className="w-4 flex justify-center"><Clock size={12} className="text-zinc-400" /></div>
                            ...
                            {activeOrder && (
                                <div className="flex items-center gap-2 mt-1">
                                    ...
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-[#FF5C00]">
                                        #{activeOrder.orderNumber || `OD-${Math.floor(Math.random() * 1000000)}`}
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="lg:flex-1 overflow-y-auto no-scrollbar space-y-4 max-h-[34vh] md:max-h-[30vh] lg:max-h-none">
                            {cart.length > 0 && (
                                <div className="flex flex-col min-w-0">
                                    <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3">Adding to Order ({cart.length})</p>
                                            ...
                                            <div key={item._id} className="p-3 bg-white rounded-xl border border-zinc-200 group md:min-w-[300px] md:max-w-[320px] md:shrink-0 lg:min-w-0 lg:max-w-none">
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <div className="relative h-10 w-10 rounded-lg bg-zinc-50 border border-zinc-100 overflow-hidden shrink-0">
                                                        ...
                                                        />
                                                        <div className="absolute inset-x-0 bottom-0 bg-black/50 text-white text-[9px] font-bold text-center leading-4">
                                                            {item.quantity}x
                                                        </div>
                                                    </div>
                                                    <div>
                                                        <p className="text-sm font-bold text-zinc-900 line-clamp-1">{item.name}</p>
                                                        <p className="text-[11px] font-semibold text-zinc-400">NRs. {item.price.toLocaleString()}</p>
                                                    </div>
                                                </div>
                                                <div className="flex flex-col items-end gap-2">
                                                    <button onClick={() => removeFromCart(item._id)} className="text-zinc-300 hover:text-rose-500 transition-colors"><Trash2 size={14} /></button>
                                                ...
                                                <div className="mt-2 pl-[52px]">
                                                    <div className="relative">
                                                        ...
                                                        <input
                                                            ...
                                                            placeholder="Add note for chef (e.g. no onion, less spicy)"
                                                            className="w-full h-8 pl-8 pr-3 rounded-lg border border-zinc-200 bg-zinc-50 text-[11px] font-medium text-zinc-700 placeholder:text-zinc-400 focus:bg-white focus:border-zinc-300 outline-none"
                                                        />
                                                    </div>
                                                    {item.notes?.trim() && (
                                                        <div className="mt-1.5 inline-flex items-center rounded px-2 py-1 text-[10px] font-semibold bg-rose-50 text-rose-700">
                                                            {item.notes.trim()}
                                                        </div>
                                                    )}
                                                </div>
                            ...
                            {activeOrder?.items?.length > 0 && (
                                <div className="flex flex-col min-w-0 space-y-3">
                                    <div className="flex items-center justify-between p-3 rounded-xl bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200">
                                        <div>
                                            <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">Order Status</p>
                                            <p className="text-sm font-bold text-blue-900 mt-1">{activeOrder.status}</p>
                                        </div>
                                        {activeOrder.status === 'COOKING' && (
                                            <Clock size={18} className="text-orange-500 animate-spin" />
                                        )}
                                    ...
                                    <div className="space-y-3">
                                        {activeOrder.items.map((item: any) => (
                                            <div key={item._id} className="flex items-center justify-between p-3 bg-white/50 rounded-xl border border-dotted border-zinc-200 opacity-70">
                                                <div className="flex items-center gap-3">
                                                    <div className="h-8 w-8 rounded-lg bg-zinc-100 flex items-center justify-center font-bold text-[10px] text-zinc-500">
                                                        {item.quantity}x
                                                    </div>
                                                    <div>
                                                        <span className="text-xs font-bold text-zinc-600 line-clamp-1 block">{item.name}</span>
                                                        {item.notes && (
                                                            <span className="inline-flex mt-1 rounded px-1.5 py-0.5 text-[9px] font-semibold bg-rose-50 text-rose-700">
                                                                {item.notes}
                                                            </span>
                                                        )}
                                                    </div>
                                                ...
                                                <div className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${
                                                    ...
                                                    {activeOrder.status === 'COMPLETED' ? 'COMPLETED' : item.status}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                        ...
                        <div className="pt-4 border-t border-zinc-100 shrink-0 bg-white">
                            <div className="space-y-3 mb-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <p className="text-sm font-bold text-zinc-900">NRs. {subtotal.toLocaleString()}</p>
                                </div>
                                <div className="flex justify-between items-center">
                                    <p className="text-sm font-bold text-zinc-500">Tax 13% (VAT)</p>
                                    <p className="text-sm font-bold text-zinc-900">NRs. {tax.toLocaleString()}</p>
                                </div>
                            </div>
                            ...
                            {activeOrder?.status === 'COMPLETED' ? (
                                <button
                                    ...
                                    disabled={activeOrder?.billPrinted}
                                    className={`h-12 w-full rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all
                                        ${activeOrder?.billPrinted 
                                            ? 'bg-emerald-500 text-white cursor-not-allowed' 
                                            : 'bg-[#FF5C00]/90 hover:bg-[#FF5C00] text-white active:scale-95'}`}
                                >
                                    <Receipt size={18} /> {activeOrder?.billPrinted ? 'Bill Printed ✓' : 'View Bill & Print'}
                                </button>
                            ) : (
                                <button
                                    onClick={sendToKitchen}
                                    disabled={!selectedTable || cart.length === 0}
                                    className="h-12 w-full bg-[#FF5C00]/90 hover:bg-[#FF5C00] text-white rounded-xl text-sm font-bold flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50 transition-all"
                                >
                                    <CheckCircle2 size={18} /> Send to Kitchen
                                </button>
                            )}
                        </div>
                    </div>
            ...
            {showBilling && activeOrder && (
                <BillingView
                    order={activeOrder}
                    onClose={() => setShowBilling(false)}
                    onBillPrinted={async () => {
                        // Refresh order data to show updated billPrinted status
                        if (selectedTable) {
                            const refreshedOrder = await fetchActiveOrder(selectedTable);
                            setActiveOrder(refreshedOrder);
                        }
            ...
            {toast && (
                <div className={`fixed bottom-6 right-6 rounded-xl shadow-lg px-5 py-3.5 text-sm font-bold flex items-center gap-3 animate-in slide-in-from-bottom-4 z-40
                    ${toast.type === 'success' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' :
                      toast.type === 'error' ? 'bg-rose-50 text-rose-700 border border-rose-200' :
                      toast.type === 'warning' ? 'bg-amber-50 text-amber-700 border border-amber-200' :
                      'bg-blue-50 text-blue-700 border border-blue-200'}`}>
                    {toast.type === 'success' && <CheckCircle2 size={16} />}
                    {toast.type === 'error' && <Trash2 size={16} />}
                    {toast.type === 'warning' && <Bell size={16} />}
